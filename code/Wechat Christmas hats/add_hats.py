...
sample_image = cv2.imread('gyy.png')
# [x, y, w, h]，(x,y)左上角起始坐标，w宽，h高
...
for face in faces:
    ...
    x1, x2 = max(x_offset, 0), \
             min(x_offset + hat.shape[1], sample_image.shape[1])

    y1, y2 = max(y_offset, 0), \
             min(y_offset + hat.shape[0], sample_image.shape[0])
...
# 保存最终结果
cv2.imwrite('res.png', sample_image)